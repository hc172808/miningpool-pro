...
const { createCryptoNode, stopCryptoNode, getCryptoNodeStatus, isNodeRunning } = require('./crypto/node-manager');
const { storage } = require('./storage');
const { MessageType } = require('./websocket');

// Create a router for nodes
function setupNodeRoutes(app) {
  // Get all node configurations
  app.get("/api/node-configurations", async (req, res) => {
    try {
  ...
  app.get("/api/node-configurations/:id", authenticate, requireAdmin, async (req, res) => {
    try {
  ...
  app.get("/api/node-configurations/coin/:coin", authenticate, requireAdmin, async (req, res) => {
    try {
  ...
  // Start a node
  app.post("/api/node-configurations/:id/start", authenticate, requireAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const nodeConfig = await storage.getNodeConfiguration(id);
  ...
  // Stop a node
  app.post("/api/node-configurations/:id/stop", authenticate, requireAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const nodeConfig = await storage.getNodeConfiguration(id);
  ...
  // Get node status
  app.get("/api/node-configurations/:id/status", authenticate, requireAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const nodeConfig = await storage.getNodeConfiguration(id);
...
module.exports = { setupNodeRoutes };